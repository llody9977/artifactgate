...
if [ "$INSECURE_LAB_MODE" = true ]; then
    ...
    if command -v cosign &> /dev/null; then
        ...
        # Semantic OpenVEX Validation (Iterating entry-by-entry over array)
        ...
        if [ -n "$APP_VEX_RAW" ]; then
            ARR_LEN=$(echo "$APP_VEX_RAW" | jq -r 'if type == "array" then length else 1 end' 2>/dev/null || echo 1)
            for idx in $(seq 0 $((ARR_LEN-1))); do
                echo "$APP_VEX_RAW" | jq -r "if type == \"array\" then .[$idx].payload else .payload end" | base64 -d 2>/dev/null | jq -r '.predicate // empty' > /tmp/app-vex.json 2>/dev/null || true
                if [ -s /tmp/app-vex.json ]; then
                    if python3 .github/scripts/validate_vex.py /tmp/app-vex.json "$GHCR_DIGEST" >/dev/null 2>&1; then
                        APP_VEX_VERIFIED=true
        ...
        if [ -n "$RUNNER_VEX_RAW" ]; then
            ARR_LEN=$(echo "$RUNNER_VEX_RAW" | jq -r 'if type == "array" then length else 1 end' 2>/dev/null || echo 1)
            for idx in $(seq 0 $((ARR_LEN-1))); do
                echo "$RUNNER_VEX_RAW" | jq -r "if type == \"array\" then .[$idx].payload else .payload end" | base64 -d 2>/dev/null | jq -r '.predicate // empty' > /tmp/runner-vex.json 2>/dev/null || true
                if [ -s /tmp/runner-vex.json ]; then
                    if python3 .github/scripts/validate_vex.py /tmp/runner-vex.json "$RUNNER_GHCR_DIGEST" >/dev/null 2>&1; then
                        RUNNER_VEX_VERIFIED=true
        ...
        # Deterministic Decision Resolution (Cross-subject decisionId matching)
        ...
        if [ -n "$APP_DEC_RAW" ] && [ -n "$RUNNER_DEC_RAW" ]; then
            APP_LEN=$(echo "$APP_DEC_RAW" | jq -r 'if type == "array" then length else 1 end' 2>/dev/null || echo 1)
            RUN_LEN=$(echo "$RUNNER_DEC_RAW" | jq -r 'if type == "array" then length else 1 end' 2>/dev/null || echo 1)

            for i in $(seq 0 $((APP_LEN-1))); do
                echo "$APP_DEC_RAW" | jq -r "if type == \"array\" then .[$i].payload else .payload end" | base64 -d 2>/dev/null | jq -r '.predicate // empty' > /tmp/app-predicate.json 2>/dev/null || true
                if [ -s /tmp/app-predicate.json ]; then
                    APP_DEC_ID=$(jq -r '.decisionId // empty' /tmp/app-predicate.json)
                    if [ -n "$APP_DEC_ID" ] && python3 .github/scripts/validate_promotion_decision.py \
                        ...
                        # Cross-check runner decision attestation for the EXACT SAME decisionId!
                        for j in $(seq 0 $((RUN_LEN-1))); do
                            echo "$RUNNER_DEC_RAW" | jq -r "if type == \"array\" then .[$j].payload else .payload end" | base64 -d 2>/dev/null | jq -r '.predicate // empty' > /tmp/runner-predicate.json 2>/dev/null || true
                            if [ -s /tmp/runner-predicate.json ]; then
                                RUN_DEC_ID=$(jq -r '.decisionId // empty' /tmp/runner-predicate.json)
                                if [ "$APP_DEC_ID" = "$RUN_DEC_ID" ] && python3 .github/scripts/validate_promotion_decision.py \
                                    --predicate-file /tmp/runner-predicate.json \
                                    --expected-app-promoted-digest "$GHCR_DIGEST" \
                                    --expected-runner-promoted-digest "$RUNNER_GHCR_DIGEST" \
                                    --expected-repository "$REPO_SLUG" \
                                    --expected-platform "linux/amd64" \
                                    --expected-decision-id "$APP_DEC_ID"; then
                                    APP_DECISION_VERIFIED=true
                                    RUNNER_DECISION_VERIFIED=true
                                    rm -f /tmp/app-predicate.json /tmp/runner-predicate.json
                                    break 2
                                fi
                            fi

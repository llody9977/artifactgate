...
5. **OIDC Promotion & Attestation**: Approved image pairs are copied to the protected `llody9977/artifactgate/n8n-trusted` registry path. The promotion workflow generates and keylessly signs SLSA build provenance, SPDX SBOM declarations, OpenVEX documents, and a signed **Promotion Decision Attestation** (`promotiondecision` predicate) recording policy hashes, evidence manifest hashes, and waiver metadata.
6. **Admission Verification & Re-scanning**: Workloads are deployed using their exact SHA256 digests. The `install.sh` installer verifies Cosign signatures, SLSA provenance, SPDX SBOMs, OpenVEX attestations, and semantic `promotiondecision` predicates (validating 4-digest pairing, policy hashes, evidence completion, and waiver expiry) for both images before initialization. Post-startup container inspections confirm running container image IDs match target digests. Deployed builds are re-scanned weekly on a cron schedule to check for post-release security decay.
